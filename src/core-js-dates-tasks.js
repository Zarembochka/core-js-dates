...
function getDayName(date) {
  ...
  return dayNames[dataDay.getDay()];
}
...
function getCountDaysInMonth(month, year) {
  const dayDate = new Date(year, month, 0);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const coundtDays = dayEnd - dayStart;
  return Math.floor(coundtDays / (1000 * 60 * 60 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return dateToCheck - periodStart >= 0 && periodEnd - dateToCheck >= 0;
}
...
function formatDate(date) {
  ...
  const month = dayData.getUTCMonth() + 1;
  const day = dayData.getUTCDate();
  const year = dayData.getUTCFullYear();
  const time = dayData.toLocaleTimeString('en-US', { timeZone: 'UTC' });
...
function getCountWeekendsInMonth(month, year) {
  const startDayDate = new Date(year, month - 1, 1);
  const lastDayDate = new Date(year, month, 0);
  const daysCount = getCountDaysOnPeriod(startDayDate, lastDayDate);
  let dayOfTheWeek = startDayDate.getDay();
  let weekendCount = 0;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const endDateArray = period.end.split('-');
  ...
  while (periodStartDate <= periodEndDate) {
    let i = 0;
    while (i < countWorkDays && periodStartDate <= periodEndDate) {
      const workDate = periodStartDate.getDate().toString();
      const workMonth = String(+periodStartDate.getMonth() + 1);
      schedule.push(
        `${workDate.padStart(2, '0')}-${workMonth.padStart(2, '0')}-${periodStartDate.getFullYear()}`
      );
      periodStartDate.setDate(periodStartDate.getDate() + 1);
      i += 1;
    }
    periodStartDate.setDate(periodStartDate.getDate() + countOffDays);
  }
  return schedule;
}
...
function isLeapYear(date) {
  ...
  const divisibleBy4 = yearToCheck % 4;
  const divisibleBy100 = yearToCheck % 100;
  const divisibleBy400 = yearToCheck % 400;
  return !divisibleBy4 && (divisibleBy100 !== 0 || !divisibleBy400);
}
